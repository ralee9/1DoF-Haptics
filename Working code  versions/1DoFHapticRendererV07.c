...
void Datachange_testmsg(struct Wixel_msg *);
...
int main(void)
{
    ...
    // Main loop --------------------------------------------------------------
		while(1) {
				...
				Datachange_testmsg(T_msg); // cycle through 3 12 bit messages
...
void Datachange_testmsg(struct Wixel_msg *T_msg)
{
	...
	T_msg->Position = counter;
	if (counter == 0xAFE)
	{
		 T_msg->Force = counter + 0x101;
		 T_msg->Output = 0xC00;
	}
	else if (counter == 0xAFF)
	{
		 T_msg->Force = 0xB00;
		 T_msg->Output = 0xC01;
	}
