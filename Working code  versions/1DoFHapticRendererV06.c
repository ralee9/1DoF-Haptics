...
// Global variables -----------------------------------------------------------
float voltageForce, voltageForce_avg, voltageSpeaker, epsilon = 0.5; 
      ...
      if (counter%1000 == 0)
			...
			*/
			...
			//message = SPI_read();         // reading data from the Wixel
				...
				if (voltageForce - voltageForce_avg < -epsilon)
				{
					//Push force, move speaker down
					voltageSpeaker += 0.001;
					DAC0DAT = outputDAC(voltageSpeaker);
				}
				else if (voltageForce - voltageForce_avg > epsilon)
				{
					//pull force, move speaker down
					voltageSpeaker -= 0.001;
					DAC0DAT = outputDAC(voltageSpeaker);
				}
				else
