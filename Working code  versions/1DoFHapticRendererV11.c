...
int main(void)
{
	...
	pPID_zero = &PID_zeroStiff; // PID controller values for zero stiffness spring
...
float PID_zeroStiffness(struct PID_const *PID) 
{
    ...
    */
    ...
    /* Linear function controller equation */
	...
	*/
    /*-------------------------------------------------------------------------
		...
		// Zero stiffness spring behavior (original implementation)
        // Push --> voltage drops, Pull --> voltage increase
		voltageForce = readSensors(0);
		
        // at voltageForce_avg -> zero force generated by speaker
		// 2V is "lowest" voltage seen when pushing on force sensor
        // empirically obtained, m = (y1-y0)/(x1-x0) form
		forceSlope = 1.25 / ( 2 - voltageForce_avg );       
        
        // "y-intercept" of linear function for speaker actuation
		intersectionPoint = 1.25 - forceSlope * voltageForce_avg;
...
/** potVoltage(): Calculates voltage on a potentiometer. Returns decimal scaled 
by fullscaleValue **/
float potVoltage(float voltage, float fullscaleValue)
{
