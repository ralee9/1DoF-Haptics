...
struct Wave_const Tri_const, *pTri_const;

/* Flags, counters, and misc ---------------------------------------------- */
/* timer flags are volatile because they are flipped by interrupt routine */
...
int mode = 0;           /* speaker mode */
int g_membrane_punctured;   /* flag for membrane puncture function */
float epsilon = 0.02;
...
/* data update and formatting functions for 3 channel SPI comm prototyping */
...
void dataUpdate_msg(struct Wixel_msg *data);    /* 8 channel SPI comms test */
...
float triangle_wave(struct Wave_const *);
...
int main(void)
{
    ...
    /* triangle wave not implemented, 23 March 2016 */
    //pTri_const = &Tri_const;
	//pTri_const->theta_counter = 0;
	...
	/* 2 second delay to allow sensors to stabilize after power on/ reset */
	delay_ms(1000);
	
	/* initial read/store of all sensors */
    //rest_pos_volts = read_store_sensors(4, pInit_data);
    ...
    /* set K_u, T_u, and PID constants for virtual wall */
    pPID_wall->K_u = 1.861;
    pPID_wall->T_u = 0.01;
	#if OBSOLETE
	...
	/* set K_u, T_u, and PID constants for zero stiffness */
    pPID_zero->K_u = 2.870;
    pPID_zero->T_u = 0.002;
	...
	while(1) {
         ...
         * timer0 currently set for 750 us --> 1.333 kHz 
         */
		timer0_init();
        ...
        /* manual control of speaker testing */
        /* WORKS, 16 March 2016 */
        speaker_volts = read_store_sensors(6, pCurr_data);
        speaker_voltageHex = outputDAC(speaker_volts);
        DAC0DAT = speaker_voltageHex;
        store_DAC_data(speaker_voltageHex, pCurr_data);

        /* sine wave generator testing */
        /* WORKS, 16 March 2016 */
        ...
        /* moving virtual wall WORKS, 18 March 2016 */
        desiredSpeaker_volts = read_store_sensors(6, pCurr_data);
        if (abs(desiredSpeaker_volts - lastDesiredSpeaker_volts > epsilon))
        {
            /* use potentiometer to choose set point from linear range between
             * 0 and 8.77 mm displacement 
             */
            desiredSpeaker_mm = pot_voltage(desiredSpeaker_volts, 8.77);
            pPID_wall->setPoint = desiredSpeaker_mm;
			pPID_wall->errorSum = 0.0;
			pPID_wall->lastError = 0.0;
        }
        else
        {
            /* render virtual wall if no change in desired speaker position */
		...
		/* virtual wall behavior testing */
		/* basic PI control WORKS, 17 March 2016 */
		speaker_volts = PID_virtualWall(pPID_wall);
        speaker_voltageHex = outputDAC(speaker_volts);
        DAC0DAT = speaker_voltageHex;
        store_DAC_data(speaker_voltageHex, pCurr_data);
...
float PID_virtualWall(struct PID_const *PID)
{
	 ...
	 */
	K_u = 1.861;	/* ultimate gain @ oscillation */
	...
	/* output eqn for when error = set - curr */
...
float PID_zeroStiffness(struct PID_const *PID) 
{
     ...
     * for the eqn: output = K_p*e + K_i*integral(e) + K_d*d/dt(e)
     */
    K_u = 2.870;		/* ultimate gain @ oscillation */
    ...
    // pState is positive to "run away" from the force on the sensor
    output_voltage = pState + iState + dState + 1.25;
...
float sine_wave(struct Wave_const *constants)
{
    ...
    /* delta_theta is fullscale / operating freq, the change in theta to  
     * obtain a 1 Hz sine wave. For 16 bits @ 1333 Hz update, delta = 49
     */
    delta_theta = 49 * frequency;
...
/** triangle_wave() --
 **     Generate a triangle wave with variable frequency and amplitude
 **/
float triangle_wave(struct Wave_const *constants)
{
    /* not yet implemented, 23 March 2016 */
}
