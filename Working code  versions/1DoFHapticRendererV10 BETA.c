...
V10BETA -- Added readData_msg() for reading on HEX
					 Try to read the force sensor/knobs and send them through send_data()
...
int main(void)
{
    ...
    // Main loop --------------------------------------------------------------
		while(1) {
				...
				S_msg->Position = readData_msg(0);
				S_msg->Force = readData_msg(1);
				S_msg->Output = readData_msg(2);
			  ...
			  // use measurements of input force sensor to move speaker away from contact
...
float readSensors(int channel)
{
	// ADC conversion, result returned in hex
	float sensorVoltage;
...
/**Reads sensor voltage from ADC channel and keep it on HEX**/
int readData_msg(int channel)
{
	int sensorVoltage;
