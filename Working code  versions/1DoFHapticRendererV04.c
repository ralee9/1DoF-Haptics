      ...
      if (counter%1000 == 0)
			...
			*/
			...
			//message = SPI_read();         // reading data from the Wixel
				...
				{
					GP2DAT ^= 0x20000;
					//GP2DAT = 0xFFF0000;		            // turn off LED
				}	
				
				//DAC2DAT = outputDAC(0.123);
...
int outputDAC(float desired_voltage)	
{
	...
	if (desired_voltage <= 2.5 && desired_voltage >= 0.036)
	{
	...
	else if (desired_voltage > 2.5)							// More then 2.5V gives 2.5V	
	{	
	...
	{
		desired_voltage = 1.2 * desired_voltage - 0.0072733;         // Liniar function between the real to the desired voltage
		output_integer = floor((desired_voltage * 0xFFF) / 2.497);    // new desired_voltage to get the real desired_voltage
		output_integer = output_integer << 16;
	}	
...
void testmode(int times)
{
	...
	for (n=1 ; n <= times ; ++n )  
	{
		for ( voltage=0.0065 ; voltage <= 2.49 ; voltage=voltage+0.001 ) // Cycle up to 2.49v
		{
			DAC2DAT = outputDAC(voltage);
			delay_ms(2);
		}	
		DAC2DAT = outputDAC(2.49);
		delay_ms(50);
		
		for ( voltage=2.49 ; voltage >= 0.0065 ; voltage=voltage-0.001 )  // Cycle down to 0.0065V
		{
			DAC2DAT = outputDAC(voltage);
			delay_ms(2);
		}
	...
	while ( times >= 0 )
	{
		...
		if ( voltage >= 2.49 ) // Cycle down to 0.0065V	
		{
			DAC2DAT = outputDAC(2.49);
		 step = -0.001;
		 delay_ms(100);
